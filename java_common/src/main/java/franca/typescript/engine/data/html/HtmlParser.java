package franca.typescript.engine.data.html;

import franca.typescript.engine.contract.StringBuffer;
import franca.typescript.engine.contract.SwiftRuntime;
import franca.typescript.engine.data.Parser;
import franca.typescript.engine.data.json.JsonArray;
import franca.typescript.engine.data.json.JsonObject;
import franca.typescript.engine.data.json.JsonStringPrimitive;
...
public class HtmlParser extends Parser {
  ...
  public void toStringBuffer(JsonArray jsonArray, StringBuffer stringBuffer) {
    for (int i = 0; i < jsonArray.count(); i++) {
      JsonObject jsonObject = jsonArray.getJsonObjectItem(i);
      if (jsonObject == null) {
        continue;
      }
